...
**Version:** 1.0
**Date:** 2026-02-26
**Status:** DRAFT — Governance review pending (Observer Council)
...
|--------|--------|--------|---------------|
| `session.create` | `{ client_type, client_id, auth_token }` | `{ session_id, created_at }` | Yes |
| `session.close` | `{ session_id }` | `{ closed: true }` | Yes |
| `session.resume` | `{ session_id, auth_token }` | `{ session_id, threads[] }` | Yes |
...
| `approval.respond` | `{ approval_id, decision, reason? }` | `{ acknowledged: true }` | Yes |
| `health.status` | `{}` | `{ server, backends{}, uptime }` | No |
| `audit.query` | `{ session_id?, thread_id?, since?, limit? }` | `{ events[] }` | Yes |
...
interface AuditLogger {
  // Log an event (synchronous write -- audit must not be lossy)
...
| Client Method | Core Target | Core Endpoint |
|--------------|-------------|---------------|
| `turn.submit` (cognitive intent) | Atlas/OIL | `core.process_turn` |
| `turn.submit` (build intent) | Atlas Builder | `core.dispatch_build` |
| `turn.submit` (query intent) | Vault I/O | `core.query` |
| `thread.cancel` | Core lifecycle | `core.cancel_thread` |
...
The router translates cross-VM errors into meaningful client-facing errors:
...
**ISC Exit Criteria:**
- Routes `turn.submit` to correct core endpoint based on intent classification
...
interface BackendStatus {
  ...
  enabled: boolean;
  healthy: boolean;
  health_state: "healthy" | "degraded" | "rate_limited" | "unavailable";
  last_check: string;
...
- Config reload works without server restart
...
interface HealthStatus {
  ...
  backends: Record<string, {
    status: "healthy" | "degraded" | "rate_limited" | "unavailable" | "disabled";
    last_check: string;
    response_time_ms: number | null;
...
**Security Considerations:**
- `health.status` is the only unauthenticated RPC method. It returns operational metrics only -- no session data, no thread content, no secrets.
...
- Health data is useful for debugging but could reveal system topology to an attacker with local access. Acceptable because the system is localhost-only and Tailscale-protected.
...
- Does not expose sensitive information in health responses
...
core:
  url: "http://100.x.x.x:9100/rpc"    # Tailscale IP of CachyOS VM
  timeout_ms: 300000                    # 5 minutes -- builds can be slow
  retry_count: 1
  retry_delay_ms: 5000
...
This section is the most critical in the specification. It integrates the Architect's foundational security design, all six Non-Negotiable Findings (NNF-1 through NNF-6) from the Security Auditor, all sixteen identified threats (T1-T16), the Sentry's kill chain requirements, and the Operator's practical concerns about secrets bootstrap. Every security decision is traced back to one of the four Security Governance Framework invariants:
...
interface DispatchResult {
  ...
  status: "completed" | "failed" | "timeout";  // Determined by dispatch layer
...
**If audit write fails (JSONL or SQLite):** The request that generated the audit event is BLOCKED. Audit is the governance backbone -- if the system cannot log an action, it must not perform the action. This is a hard requirement per the Security Governance Framework.

### 5.6 Threat Model

All sixteen identified threats with severities, attack vectors, mitigations, and residual risk assessment.
...
| **Tailscale device key** | Can connect to both VMs as authorized device. Can reach control plane on port 9000 and core on port 9100. | Still needs per-service auth tokens. Cannot access services without Bearer token. | **Low-Medium** | Per-service Bearer token auth provides independent authentication layer. |
...
| Audit write failure blocks the request | **Compliant** | If audit cannot be written, the action is not performed. |
...
**Framework:** `bun test` (built into bun, jest-compatible API). Tests run in the bun runtime for speed; production runs on Node.js 22 LTS.
...
| **Router** | Routing table, timeout handling | Routes to correct core endpoint based on intent. Timeout returns error after configured period. Retry logic for transient failures. Health check detects core unavailability. | 1 (required) |
  ...
  describe('end-to-end: child process receives no credentials', () => {
    it('dispatched process env contains no API keys', async () => {
      ...
      const result = await dispatch({
        ...
        target_override: 'env-printer-test-backend',
...
Step 5: Database integrity check
  ...
  - If integrity check fails, log error and exit 1
...
```bash
# Control plane health (unauthenticated -- this is the only public method)
...
sqlite3 data/audit.db "SELECT ts, method, status FROM events ORDER BY ts DESC LIMIT 20"

# Events for a specific session
sqlite3 data/audit.db "SELECT * FROM events WHERE session_id='sess_abc123' ORDER BY ts"

# Failed dispatches in the last 24 hours
sqlite3 data/audit.db "SELECT * FROM events WHERE method='dispatch' AND status='failed' AND ts > datetime('now', '-24 hours')"
```
...
DB="packages/control-plane/data/audit.db"

echo "=== Observer Activity (last $DURATION) ==="
echo ""

echo "--- Sessions ---"
sqlite3 "$DB" "SELECT session_id, client_id, created_at, status FROM sessions WHERE created_at > datetime('now', '$MODIFIER') ORDER BY created_at DESC"

echo ""
echo "--- Dispatches ---"
sqlite3 "$DB" "SELECT dispatch_id, backend_used, status, execution_time_seconds FROM dispatch_results WHERE ts > datetime('now', '$MODIFIER') ORDER BY ts DESC"

echo ""
echo "--- Approvals ---"
sqlite3 "$DB" "SELECT item_id, tier, decision, responded_at FROM approvals WHERE requested_at > datetime('now', '$MODIFIER') ORDER BY requested_at DESC"

echo ""
echo "--- Errors ---"
sqlite3 "$DB" "SELECT ts, method, error_code, error_message FROM events WHERE status='error' AND ts > datetime('now', '$MODIFIER') ORDER BY ts DESC LIMIT 20"
```
...
# 4. Revoke old token immediately (no restart required)
OLD_TOKEN_HASH=$(echo -n "$OLD_TOKEN" | sha256sum | awk '{print $1}')
curl -s http://127.0.0.1:9000/rpc \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $NEW_TOKEN" \
  -d "{\"jsonrpc\":\"2.0\",\"method\":\"admin.revoke_token\",\"params\":{\"token_hash\":\"$OLD_TOKEN_HASH\"},\"id\":1}"

# 5. Update client configuration to use the new token
```

The `scripts/rotate-token.sh` script automates steps 1-4. The exposure window is the time between compromise detection and running the script.
...
```
Week 1:  S0 (shared types and schemas)
Week 2:  S1 + S2 + S3 + S6 + S7 (parallel -- 5 independent slices)
Week 3:  S4 (approval gateway, depends on S1/S2/S3)
Week 4:  S5 (server + router, integration layer)
Week 5:  Cross-VM integration testing, systemd setup, operational scripts
```

This schedule assumes solo development. The parallel group can be worked serially (one slice at a time) without changing the dependency order.
...
### Q7: Audit Format -- JSONL Primary + SQLite Index?

**Context:** The Security Auditor's NNF-4 requires structurally append-only audit. SQLite's "append-only" is application-enforced (the process has full write access). A JSONL file is structurally append-only (open with O_APPEND, fsync after each write).

**Architect recommendation:** SQLite for queryability, with the understanding that append-only is application-enforced.

**Security Auditor finding:** NNF-4 (High). Use JSONL as the primary append-only audit trail. SQLite as a rebuildable queryable index on top.

**Operator input:** Adam needs `sqlite3` queries for debugging. JSONL is harder to query from a phone.

**Recommended approach (synthesized):**

1. Primary: JSONL file, opened with O_APPEND, fsync per write. Structurally append-only.
2. Secondary: SQLite database that indexes the JSONL records for querying. This database can be rebuilt from the JSONL file at any time. If the SQLite index is corrupted, the JSONL source of truth survives.
3. Operational scripts (`recent-activity.sh`, etc.) query SQLite for convenience.

Both files live in `data/` and are backed up by ZFS snapshots. Both use WAL mode (SQLite) / O_APPEND (JSONL) for crash safety.

**Default if Adam does not weigh in:** JSONL primary + SQLite index. This satisfies NNF-4 while preserving the operator's queryability needs.

---

### Q8: Telegram Client -- In This Project or Separate?

**Context:** The Telegram client is architecturally simple (thin adapter, long polling, no business logic) but is a distinct deployment unit on the Relay VM.

**Architect recommendation:** Separate project or a separate slice within this project. The Telegram client has no dependency on the control plane's internals -- it is a JSON-RPC client that happens to bridge Telegram messages.

**Operator input:** If it is in the monorepo, it benefits from shared types. If separate, it is one more repo to manage.

**Default if Adam does not weigh in:** Include as a package in the monorepo (`packages/telegram-client/`). It shares the S0 types and can be deployed independently to the Relay VM. It is Phase 2 scope -- the control plane works without it (Adam uses curl/CLI in Phase 1).
...
*This document synthesizes the Architect's initial draft with review findings from the Operator, Sentry, and Security Auditor. All synthesis decisions are documented in the section headers and Appendix A. Adam retains final authority on all open questions in Section 9.*
