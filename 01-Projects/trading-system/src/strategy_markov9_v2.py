"""
MarkovRegime9v2 — v6.2c DIRAllocator + K=9 Sizing Modulator.

Entry/exit logic: identical to v6.2c DIRAllocator (HMM gatekeeper,
momentum selection, no basin-based gates). K=9 topology is used ONLY
to scale position size via custom_stake_amount().
...
Architecture:
  ...
  Layer 4 (NEW):        K=9 basin → sizing scalar (0.1 to 1.2)

The sizing scalar combines:
  - Markov confidence: P(→bullish) / P(→bullish + →bearish)
  - Flicker penalty:   exp(-0.5 * basin_changes_in_12h)
  - Basin overrides:   C1 drawdown cap, C4→C3 bifurcation bonus
"""
...
_TRADING_ROOT = _STRATEGY_FILE.parent.parent
_DATA_DIR = _TRADING_ROOT / "data"
_MODELS_DIR = _TRADING_ROOT / "freqtrade" / "user_data" / "models"
...
BIFURCATION_BONUS = 1.2      # sizing boost for C4→C3 transition
...
class MarkovRegime9v2(IStrategy):
    ...
    # ================================================================
    # Entry/Exit — IDENTICAL to v6.2c DIRAllocator
    # ================================================================
    ...
    # ================================================================
    # K=9 Sizing Modulator (THE ONLY NEW LOGIC)
    # ================================================================

    def custom_stake_amount(self, pair, current_time, current_rate, proposed_stake,
                            min_stake, max_stake, leverage, entry_tag, side, **kwargs) -> float:
        """Scale position size based on K=9 basin stability."""
        scalar = self._sizing_scalar.get(pair, 1.0)
        scaled = proposed_stake * scalar
    ...
    def _compute_sizing_scalars(self, dataframe: pd.DataFrame, pair: str) -> pd.DataFrame:
        ...
        for i in range(n):
            ...
            # Bifurcation bonus: C4→C3 in recent history
            if basin == 3 and len(hist_list) >= 2:
                if 4 in hist_list[:-1]:
